#!/usr/bin/env python3
"""Story-driven Blender renderer that consumes the timed visual plan."""
...
def configure_shot(cam,t,shot,c,r):
 k=(shot.get('camera') or 'medium shot').lower(); d=r*(5 if 'wide' in k else 2 if 'close' in k else 2.8); h=r*(-.15 if 'low' in k else .35 if 'over-the-shoulder' in k else .5); cam.data.lens=42 if 'wide' in k else 65 if 'close' in k else 52; t.location=c+Vector((0,0,r*.12)); cam.location=t.location+Vector((d*.85,-d,h)); return (shot.get('camera_move') or '').lower()
def key_shot(cam,t,start,end,move):
 cam.keyframe_insert(data_path='location',frame=start); t.keyframe_insert(data_path='location',frame=start)
 if 'push' in move: cam.location*=.88
 elif 'pull' in move: cam.location*=1.12
 elif 'track' in move: cam.location.x+=.7
 t.location.x+=.12; cam.keyframe_insert(data_path='location',frame=end); t.keyframe_insert(data_path='location',frame=end)
def setup(args,fd):
 sc=bpy.context.scene; sc.render.engine='BLENDER_EEVEE'; sc.render.resolution_x=args.resolution_x; sc.render.resolution_y=args.resolution_y; sc.render.resolution_percentage=100; sc.render.fps=args.fps; sc.render.image_settings.file_format='JPEG'; sc.render.image_settings.quality=90; sc.render.filepath=os.path.join(fd,'frame')
def main():
 ap=argparse.ArgumentParser(); ap.add_argument('--story',required=True); ap.add_argument('--scene',required=True,type=int); ap.add_argument('--out',required=True); ap.add_argument('--fps',type=int,default=15); ap.add_argument('--resolution-x',type=int,default=540); ap.add_argument('--resolution-y',type=int,default=960); ap.add_argument('--engine',default='BLENDER_EEVEE'); ap.add_argument('--samples',type=int,default=8); ap.add_argument('--max-seconds',type=int,default=90); ap.add_argument('--no-encode',action='store_true'); argv=sys.argv; argv=argv[argv.index('--')+1:] if '--' in argv else argv[1:]; args=ap.parse_args(argv)
 if bpy is None: sys.exit('[render] bpy unavailable')
 story=json.load(open(args.story)); scene=story['scenes'][args.scene]; duration=max(1,min(int(scene.get('duration_seconds',20)),args.max_seconds)); total=max(2,int(duration*args.fps)); clear(); world(scene.get('mood','neutral')); environment(scene); lights(scene.get('mood','neutral'))
 aid=os.path.join('assets','animations',story.get('id','story')); load(scene.get('animation_file') or os.path.join(aid,f"scene{scene['id']}.fbx")) or load(scene.get('character_file') or 'assets/characters/hero.fbx') or placeholder()
 c,r=bounds(); cam,t=camera(); plans=[p for p in story.get('visual_plan',[]) if int(p.get('scene_id',-1))==int(scene['id'])]; shots=plans[0].get('shots',[]) if plans else []
 if not shots: shots=[{'start':0,'duration':duration,'camera':scene.get('camera','medium shot'),'camera_move':'slow_push_in','narration_anchor':''}]
 for shot in shots:
  start=max(0,float(shot.get('start',0))); end=min(duration,start+float(shot.get('duration',duration))); sf=max(1,int(start*args.fps)+1); ef=max(sf+1,int(end*args.fps)); move=configure_shot(cam,t,shot,c,r); key_shot(cam,t,sf,ef,move); log(f"shot {shot.get('id',0)} {start:.2f}-{end:.2f}s {shot.get('camera')} :: {shot.get('narration_anchor','')[:80]}")
 bpy.context.scene.frame_start=1; bpy.context.scene.frame_end=total; fd=os.path.join(os.path.dirname(args.out) or '.',f'.frames_scene{scene["id"]}'); shutil.rmtree(fd,ignore_errors=True); os.makedirs(fd); setup(args,fd); log(f'rendering scene {scene["id"]}: {len(shots)} timed shots'); bpy.ops.render.render(animation=True)
 try:
  os.chmod(fd,0o777)
